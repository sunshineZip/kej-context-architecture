...
# changed to an `if` instead, because the sync-check step below needs to
# run on every path through this script, not just the one where
# PowerShell needs installing. An early exit here would have silently
# skipped that sync on every local session and every cached remote
# container — i.e. almost always.
