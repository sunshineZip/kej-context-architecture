...
> **This fork already has a concrete implementation of this pattern** — this document *is* the generalization of it, in fact (see the Version History below). The companion repo is `restricted/` (submodule → `kej-restricted-context-architecture`); `restricted/README.md` is the equivalent of this file's Setup Checklist, already filled in with this fork's real names and decisions rather than placeholders. Kept here anyway for two reasons: so a future upstream sync can diff against the canonical version instead of losing track of where this fork stands relative to it, and as the generic reference if this pattern is ever reused elsewhere.
