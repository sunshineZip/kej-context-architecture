...
> **This fork's concrete implementation is `grandfather-review/`** (queue.md + log.md), naming KEJ as the external authority — this file is the generalization of that pattern, relayed upstream via `[FLAG FOR UPSTREAM]`. The fork's own file names predate and differ from this document's generic `external-review-queue.md`/`external-review-log.md` naming; that's expected, not a gap to fix. Kept here for the same reason as `restricted-tier.md`'s equivalent note.
