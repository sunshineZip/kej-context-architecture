...
Version 1.16 | 2026-08-09 | Production
...
- **`boe-slaegten-fra-mors-og-fur-2024-media/`** — a subfolder holding images extracted from the `.docx` (48 real images embedded in it, all 48 now checked, 47 physically stored). See that folder's own `manifest.md` for the per-image inventory — kept separate from this manifest because the schema is different (visual description, legibility/confidence, people depicted) and would otherwise dwarf the text extraction log below. Only `image18` (a confirmed exact duplicate of `image2`) is not stored. `image26`/`image27` were briefly misidentified as belonging to a different manuscript (KEJ's Hopp-slægten project) based on their embedded folder paths, and moved out; the human corrected this directly (2026-08-06) — both are Boe-slægten family photos and are filed here.
...
| 1.15 | 2026-08-06 | Correction: image26/image27 are Boe-slægten family photos, not Hopp-slægten — the human identified both directly (Hanne Boe with Knud Erik Jakobsen; a 1995 family group with six identified individuals plus one, Astrid Kirk, not yet linked in `tree.ged`). Moved back into this deep well's own media folder. 47 of 48 images now physically stored — only the confirmed duplicate, image18, remains unstored. |
| 1.16 | 2026-08-09 | **Source manuscript moved to `restricted/deep-wells/`** (Phase 1, `restricted/remediation-plan-2026-08-09.md`) — an audit found the unredacted `.docx` and its plain-text extraction sitting in the public repo's live working tree, undoing the point of redacting the derived content. This manifest, the extraction log, and the media subfolder's own manifest all stay here and public — only the two raw source files moved. |
