...
- [x] **Individual/family extraction plan (decided 2026-08-05, started 2026-08-06):** `family-tree/tree.ged` created and built incrementally, one generation/branch at a time as each is read — generations "02"-"05" and the full Efterslægt (descendants, p.25-54, including branch g's five checkpoints) of Chapter 1 are now fully read: 240 individuals / 85 families, append-only IDs per `context/data-structure-proposal.md` §4.1. Chapter 1's family-tree read-through is complete. Chapters 2-4 (Aneoversigt/Kapitel 2 onward) and the anetavle appendices (e.g. "SØGÅRD-SLÆGTEN," "KAPITEL 9") remain unread. The extraction file (`context/boe-slaegten-chapter1-extraction.md`) cross-references the specific `@I@`/`@F@` IDs each section produced.
