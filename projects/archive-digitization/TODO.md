...
- [ ] **Hopp-slægten reading, phased (2026-08-08, revised same day):** first three chapter files in hand (01, 01C, 01F — see `library/deep-wells/hopp-slaegten-manifest.md`). **Phase 1, Checkpoint 1 (done):** 01's surviving overview/index table (root chain, 6 generations) plus 01C's own generation (Niels Jørgen Knudsen Hopp, wife, 6 children) — 34 individuals into `family-tree/tree.ged`. **Scope correction:** 01C alone turned out comparable in size to the *entire* Boe-slægten manuscript, not one chapter — each of the 6 children has their own children documented in full, and further generations beyond that. Phase 1 is now itself multi-checkpoint: each child's descendants (and theirs) are separate future checkpoints, not one remaining pass. **Phase 2** (own session, not started): 01F's full 91 pages (Knud Knudsen Hopp, d.1762) — the largest file received so far; a stopgap stub from an external secondary source is already in `tree.ged` (`#I384`) pending this. `projects/archive-digitization/context/hopp-slaegten-kapitel1-extraction.md` has the full extraction notes.
