...
- [ ] **Hopp-slægten reading, phased (2026-08-08, revised twice same day):** first three chapter files in hand (01, 01C, 01F — see `library/deep-wells/hopp-slaegten-manifest.md`). **Phase 1, Checkpoint 1 (done):** 01's surviving overview/index table (root chain, 6 generations) plus 01C's own generation (Niels Jørgen Knudsen Hopp, wife, 6 children) — 34 individuals. **Checkpoint 2 (done, full depth):** one full child-line (Dorthe Nielsdatter Hopp's daughter Anne Cathrine) — 230 individuals, 60 living. This prompted the **"our line" depth policy**: the chapter numbering (01→02→...→07, always via child "a," landing on KEJ's own surname Jakobsen) shows 01C/01F are side branches, not the chain toward KEJ — full depth now reserved for the actual direct-line chapters (02 onward, whenever received), side branches within 01C/01F capped (children+grandchildren of the file's subject modeled, deeper generations summarized unless short). Checkpoint 2 left as-is, not trimmed. **Checkpoints 3-4 (done, capped):** Dorthe's remaining children II.-V. (25 individuals) and Berthe's children + Knud Nielsen Hopp's children I.-III. of 10 (10 individuals) — Knud Nielsen Hopp's children IV.-X. still unread, honest gap. **Phase 2** (own session, not started): 01F's full 91 pages (Knud Knudsen Hopp, d.1762) — a stopgap stub from an external secondary source is already in `tree.ged` (`#I384`) pending this. `projects/archive-digitization/context/hopp-slaegten-kapitel1-extraction.md` has the full extraction notes and policy detail.
