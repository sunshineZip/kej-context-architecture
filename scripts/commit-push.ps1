...
#
# Note on remote integration: this repo has more than one writer (the human
# also pushes directly) — see knowledge/flow/git-collaboration.md. This
# script fetches and rebases onto origin before every push, and re-validates
# even after a clean rebase, because two edits to different files (or
# non-adjacent sections of the same file) can merge with no git conflict at
# all and still leave something structurally inconsistent — that's
# validate.ps1's job to catch, not git's. (Two sessions colliding on the
# exact same append point, like a session-log turn, is more likely to
# surface as a real rebase conflict than to merge silently.) On any real
# conflict, or on a validation failure after a clean rebase, this script
# stops without pushing rather than guessing at a resolution. It never
# force-pushes under any circumstance — that always requires a human
...
#     anyone who runs this script, hooksPath set or not. -AllowBranch is
#     the deliberate, visible override — same philosophy as the hooks'
#     --no-verify, never a silent bypass. ---
$defaultBranch = "main"
