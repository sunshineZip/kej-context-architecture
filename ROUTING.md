...
- **Triaging a file out of `incoming/` isn't finished until the file is gone from `incoming/`.** As of 2026-08-08: moving it to its real home, or discarding it as a registry-only citation, is only half the step — remove it from `incoming/` in the same session, not as a separate later cleanup. This isn't destructive even for the discard case: this repo has never rewritten git history, so anything ever committed to `incoming/` stays recoverable via `git log`/`git show` regardless. `scripts/validate.ps1` warns (not errors) if `incoming/` is non-empty, as a nudge to check whether triage is overdue — it does not try to match individual files against `intake-manifest.md`.
